...
When monitoring CPU usage, remember that Redis is single\-threaded\. Thus, multiply the reported CPU usage by the number of CPU cores to get the actual usage\. For example, a four\-core CPU reporting a 20\-percent usage rate actually means that one core is running at 80 percent utilization\.
