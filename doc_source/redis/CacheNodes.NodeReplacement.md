...
+ **Do nothing** – If you do nothing, ElastiCache replaces the node as scheduled\. 
  ...
  For non\-Cluster configurations with auto\-failover enabled, clusters on Redis 5\.0\.5 complete replacement while the cluster continues to stay online and serve incoming write requests\. For auto failover enabled clusters on Redis 5\.0\.4 or below, you might notice a brief write interruption of up to a few seconds associated with DNS updates\. 
