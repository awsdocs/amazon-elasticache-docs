...
+ Support for asynchronous flushes and deletes\. ElastiCache for Redis supports commands like `UNLINK`, `FLUSHDB` and `FLUSHALL` to run in a different thread from the main thread thereby improving performance and response times for your applications by freeing memory asynchronously\.
