...
| GlobalDatastore replica lag | This is the lag between the secondary Region's primary node and the primary Region's primary node\. For cluster mode enabled Redis, the lag indicates the maximum delay among the shards\.  | Seconds | 
