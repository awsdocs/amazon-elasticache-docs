# Creating a Redis \(cluster mode enabled\) Cluster with Replicas from Scratch<a name="Replication.CreatingReplGroup.NoExistingCluster.Cluster"></a>
...
To create a Redis \(cluster mode enabled\) cluster, see [Creating a Redis \(cluster mode enabled\) Cluster \(Console\)](Clusters.Create.CON.RedisCluster.md)\. Be sure to enable cluster mode, **Cluster Mode enabled \(Scale Out\)**, and specify at least two shards and one replica node\.

## Creating a Redis \(cluster mode enabled\) Cluster with Replicas from Scratch \(AWS CLI\)<a name="Replication.CreatingReplGroup.NoExistingCluster.Cluster.CLI"></a>
...
The preceeding command generates the following output\.
...
The following operation creates the Redis \(cluster mode enabled\) replication group `new-group` with two node groups/shards \(\-\-num\-node\-groups\)\. Unlike the preceeding example, each node group is configured differently from the other node node group \(\-\-node\-group\-configuration\)\.
...
The preceeding operation generates the following output\.
...
## Creating a Redis \(cluster mode enabled\) Cluster with Replicas from Scratch \(ElastiCache API\)<a name="Replication.CreatingReplGroup.NoExistingCluster.Cluster.API"></a>
